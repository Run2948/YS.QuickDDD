...
namespace Quick.Core
{
    ...
    public static class CommonHelper
    {
        ...
        /// 访问量
        /// </summary>
        public static long InterviewCount { get; set; } = WebExtension.GetDbContext<DefaultDbContext>().Interview.Count();

        /// <summary>
        /// 网站运营开始时间
        /// </summary>
        public static DateTime RunningStart { get; set; } = WebExtension.GetDbContext<DefaultDbContext>().Interview.Min(i => i.ViewTime);
        ...
        public static void SetAppSettings(string key, string value)
        {
            ...
            xDoc.Save(HttpContext.Current.Server.MapPath("~/Config/appSettings.config"));
        }
        ...
        public static string ReplaceImgSrc(string content)
        {
            ...
            foreach (Match m in mc)
            {
                ...
                if (src.StartsWith("/"))
                {
                    var path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, src.Replace("/", @"\").Substring(1));
                    if (File.Exists(path))
                    {
